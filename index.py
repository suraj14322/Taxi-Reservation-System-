...
with sqlite3.connect('bhan.db') as db:
    c = db.cursor()
...
class travel:
        ...
        def Receiptt():
            if reset_counter is 0 and Firstname.get()!="" and Surname.get()!="" and Address.get()!="" and Postcode.get()!="" and Mobile.get()!="" and Telephone.get()!="" and Email.get()!="":
                def database():
                    ...
                    em=Email.get()
                    conn=sqlite3.connect('passnger_details3.db')
                    cursor=conn.cursor()
                    cursor.execute('CREATE TABLE IF NOT EXISTS passanger (ID INTEGER PRIMARY KEY AUTOINCREMENT,First_name TEXT,Surname TEXT,Address TEXT, Postcode TEXT,Email TEXT,Total Text)')
                    cursor.execute('INSERT INTO Passanger(First_name ,Surname ,Address, Postcode,Email,Total)VALUES(?,?,?,?,?,?)',(first,sur,add,post,em,total))
                    conn.commit()
                    conn.close()
                    ms.showinfo('Booked','succesfully booking done')
                database()
                self.txtReceipt1.delete("1.0",END)
                self.txtReceipt2.delete("1.0",END)
   ...
   #================================================mainframe========================================================================
        ...
        self.lblTitle=Label(Tops,font=('arial',70,'bold'),text=" Taxi Booking System Dwarhat ")
        self.lblTitle.grid()
